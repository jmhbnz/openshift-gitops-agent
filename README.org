...
openshift-gitops-operator-controller-manager-b68c79dcf-nx6d4   2/2     Running   0          18m
#+end_example
...
Next we issue a certificate that will be used to expose the Principal outside of the cluster. In this demo we will be using an OpenShift Route to expose the Principal.
...
#+begin_example
NAME                                      READY   STATUS    RESTARTS   AGE
argocd-agent-principal-58687b848c-jbd2m   1/1     Running   0          35h
argocd-dex-server-55cb6b974d-6hxp5        1/1     Running   0          37h
argocd-redis-7bd6ddbb8d-rgzdn             1/1     Running   0          37h
argocd-repo-server-67fd796f54-tdh42       1/1     Running   0          37h
argocd-server-7cf5446d9-b7v8v             1/1     Running   0          35h
#+end_example
...
#+begin_example
namespace/argocd unchanged
appproject.argoproj.io/crl-cer configured
argocd.argoproj.io/argocd unchanged
#+end_example
...
export SUBDOMAIN=$(oc --context principal get dns cluster --output jsonpath='{.spec.baseDomain}')
kustomize build 2-agent/ | envsubst | oc --context crl-cer-1 apply --filename -
#+end_src
...
#+RESULTS:
#+begin_example
application.argoproj.io/bgd-dev configured
#+end_example
